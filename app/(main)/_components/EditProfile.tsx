...
const EditProfile = ({ onClose }: EditProfileProps) => {
    ...
    Backend.from('profiles')
      ...
      .then(({ data }) => {
        if (data) {
          ...
          setCurrently((data.currently as CurrentlyData) || {});
        }
        setLoading(false);
      });
  }, [user]);
  ...
  const handleAvatarPick = async () => {
    ...
    try {
      ...
      const { data: urlData } = Backend.storage.from('recommendation-images').getPublicUrl(path);
  ...
  const handleSave = async () => {
    if (!user) return;
    setSaving(true);
    const { error } = await Backend.from('profiles')
      .update({
