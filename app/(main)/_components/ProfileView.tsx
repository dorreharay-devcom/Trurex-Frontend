...
const ProfileView = () => {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    const { data } = await Backend.from('profiles')
      .select('*')
      .eq('user_id', user.id)
      .single();
    ...
    setProfile({
      ...
      handle: data?.handle ? `@${data.handle}` : `@${user.email?.split('@')[0] || 'user'}`,
      bio: data?.bio || '',
      location: data?.location || '',
      avatarUrl: data?.avatar_url ?? null,
      trustScore: 87,
      rexCount: MOCK_RECS.length,
      followers: 142,
      following: 89,
      currently: (data?.currently as ProfileData['currently']) || {},
    });
    setLoading(false);
  }, [user]);
