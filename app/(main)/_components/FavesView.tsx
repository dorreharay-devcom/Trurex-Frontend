...
const GridCard: React.FC<{ rec: Recommendation }> = ({ rec }) => (
  <TouchableOpacity
    activeOpacity={0.85}
    className="w-[47.5%] rounded-xl overflow-hidden bg-card border border-border"
  >
...
const EmptyState: React.FC = () => (
  <View className="items-center py-16 gap-3">
    <Bookmark size={40} color={Theme.colors.muted} />
...
const FavesHeader: React.FC<FavesHeaderProps> = ({ viewMode, onViewModeChange }) => (
  <View>
    <View className="flex-row items-center justify-between px-4 pt-6 pb-5">
      <Text className="text-lg font-bold text-foreground">My Faves</Text>
      <Button
    ...
    <View className="mb-6">
      <Text className="text-sm font-semibold text-foreground px-4 mb-3">Collections</Text>
      <ScrollView
        horizontal
