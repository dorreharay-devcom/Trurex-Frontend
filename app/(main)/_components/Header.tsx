...
export const Header: React.FC<HeaderProps> = ({ searchQuery, onSearchChange, onProfilePress, onAddPress }) => (
  <View className="bg-card border-b border-border">
    <View className={`px-4 ${isWeb ? 'max-w-[1200px] w-full self-center' : ''}`}>
      <View className="h-16 flex-row items-center gap-4">
        ...
        <View className="flex-1 items-center">
          <View className={`w-full relative justify-center ${isWeb ? 'max-w-[448px]' : ''}`}>
            ...
            <TextInput
              ...
              className="pl-10 pr-4 py-2 rounded-lg bg-muted border border-border text-sm text-foreground"
            />
          </View>
        </View>
