import { RELATIONSHIP_STATUS, type RelationshipStatus } from '~/shared/config/relationshipStatus';
...
function coerceUnknownArray(value: unknown): unknown[] | null {
...
export function coerceStringList(value: unknown): string[] {
  const raw = coerceUnknownArray(value);
  if (raw == null) return [];
...
const RELATIONSHIP_STATUS_VALUES = new Set<string>(Object.values(RELATIONSHIP_STATUS));

export function parseRelationshipStatus(value: unknown): RelationshipStatus | null {
  if (typeof value !== 'string') return null;
  if (!RELATIONSHIP_STATUS_VALUES.has(value)) return null;
