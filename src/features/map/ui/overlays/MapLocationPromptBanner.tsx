...
function locationPinEmojiStyle() {
  if (isIos) return { lineHeight: 22 };
  return {};
}

const LOCATION_PIN_EMOJI_STYLE = locationPinEmojiStyle();

const MapLocationPromptBanner = ({ visible, onAllow, onNotNow }: Props) => {
      ...
      <View className="rounded-2xl border border-border bg-card p-4 shadow-md">
        <View className="mb-2 flex-row items-center gap-2">
          <View className="h-6 w-5 items-center justify-center overflow-visible">
            <Text accessible={false} className="text-[17px]" style={LOCATION_PIN_EMOJI_STYLE}>
...
export default MapLocationPromptBanner;
